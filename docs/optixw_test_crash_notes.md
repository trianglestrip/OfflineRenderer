...
- 运行 `optixw_test.exe` 或 `cornell_box_var_test.exe` 时在首帧内崩溃，退出码 `-1073741819` (ACCESS_VIOLATION)。
...
- **与 launch 规模无关**：`cornell_box_var_test.exe 1x1`（1 像素、1 射线）同样崩溃，说明问题不在大 buffer 或大 launch，而在 trace 管线/驱动或单线程内核逻辑。
...
4. **1x1 仍崩**：用 `cornell_box_var_test.exe 1x1` 可复现，便于在调试器中单步到崩溃点。

## 建议下一步

1. **用 Visual Studio 调试**：F5 运行 `optixw_test` 或 `cornell_box_var_test 1x1`，崩溃时查看调用栈与故障地址，区分是 host 端 (vector/cudaMemcpy) 还是 CUDA/OptiX 驱动内部。
...
- `renderer_loop.cpp`：保留 wavefront/launch 缓冲区的非空校验；在拷贝 launch params 后增加 `cudaStreamSynchronize(0)`，再调用 `optixLaunch`，避免拷贝未完成即启动。
...
- 测试：`cornell_box_var_test` 支持 `small`（256x256）、`box`（box 场景）、`1x1`（1×1 分辨率，用于隔离 trace 崩溃）。
