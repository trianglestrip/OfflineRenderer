...
void Denoiser::denoise(CUdeviceptr input, CUdeviceptr output, CUdeviceptr albedo, CUdeviceptr normal, uint32_t numSamples) {
    ...
    // Copy and normalize albedo guide (if provided and enabled)
    if (m_config.useAlbedo && albedo && m_d_denoiseAlbedo) {
        // Normalize by number of samples
        if (numSamples > 1) {
            // TODO: Add kernel to divide by numSamples
            // For now, just copy
            CUDA_CHECK(cudaMemcpy(
    ...
    // Copy and normalize normal guide (if provided and enabled)
    if (m_config.useNormal && normal && m_d_denoiseNormal) {
        // Normalize by number of samples and normalize vectors
        if (numSamples > 1) {
            // TODO: Add kernel to divide and normalize
            // For now, just copy
            CUDA_CHECK(cudaMemcpy(
