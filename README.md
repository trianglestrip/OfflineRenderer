...
* **后处理**: OptiX Denoiser 降噪（现在会利用 albedo 和 normal 作为指导，以减少误抖动）

### 与 libVLR 对比还需改进 ⚠️
虽然 libOptixW 目前核心功能已经实现，但与成熟的 libVLR 实现相比仍有明显差距，后续重点改进方向包括：

* **光谱渲染**：当前仅支持 RGB，缺乏全光谱采样和 Meng‑Simon 光谱转换。libVLR 提供完整蒙特卡洛光谱管线。
* **BSDF 系统**：仅有基础 Lambertian 和 emissive，尚未实现 Specular、GGX、混合 BRDF/BTDF、微表面模型等，且缺乏节点化材质编辑器。
* **纹理功能**：没有法线贴图、高度贴图、Alpha 纹理或其他多通道纹理支持。
* **光线传输算法**：仅包含基本路径追踪，缺少 LVC-BPT、双向路径追踪等高级算法。
* **场景表达**：当前仅支持单层几何体，缺乏实例化、场景图结构和动态对象。
* **相机模型**：仅实现简单透视相机，无景深、环境相机等高级功能。
* **其它渲染特性**：环境光/IES 贴图、体积光散射、光子映射等高级特性在 libVLR 中已有示例。

后续开发将以这些差距为参考，通过逐步补齐功能来缩小与 libVLR 的差距，并保持现代 C++ 与 OptiX 8 架构优势。
